	...
	while( 1 ) {
		...
		if ( can_check_message() ) {
			...
			switch( msg_rx.id ) {
				...
				case ACTIVATE_MSG: // duration in milliseconds, 32 bit, MSB first
					drl_activate(
							((uint32_t)msg_rx.data[0] << 24) |
							((uint32_t)msg_rx.data[1] << 16) |
							((uint32_t)msg_rx.data[2] <<  8) |
							((uint32_t)msg_rx.data[3] <<  0));
				break;
